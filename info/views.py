...
def face_comparing(request,app):
    ...
    try:
        ...
        uname_var = 'Harivardan' #input() #we used this to limit the user activity within the session 'Sanjith'
        
        img_file1=str(directory)+'\\static\\info\\images\\'+uname_var+'.jpg' #ur local files path
        ...
        if(cmp_.confidence>75):
            ...
            return render(request,'info/success.html')
                # HttpResponse('Same faces')
    except:
        print('not matching')
        return   render(request,'info/failure.html')
